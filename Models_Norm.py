...
def get_graph_feature(x, args, k=20, idx=None):
    ...
    idx_base = torch.arange(0, batch_size, device=device).view(-1, 1, 1) * num_points  # batch  (batch_size, 1, 1)
...
class PointNet(nn.Module):
    def __init__(self, args, num_class=10):
        ...
        self.posreg_C1 = linear_classifier(1024, 4)
        self.posreg_C2 = linear_classifier(1024, 4)
    ...
    def forward(self, x, alpha=0, activate_DefRec=False):
        ...
        cls_logits["pos_cls1"] = self.posreg_C1(x)
        cls_logits["pos_cls2"] = self.posreg_C2(x)
        cls_logits["def_cls"] = self.defcls_C(x)
        # cls_logits["curv_conf"] = self.curvconfreg_C(x)
        cls_logits["norm_reg"] = self.normreg_C(x5.squeeze(3)).permute(0, 2, 1)
...
class DGCNN(nn.Module):
    def __init__(self, args):
        ...
        self.defcls_C = linear_classifier(2048, 27)
        self.normreg_C = nn.Conv1d(1024, 4, kernel_size=1, bias=False)
        # self.curvconfreg_C = linear_classifier(1)
        self.DecoderFC = DecoderFC(args, 2048)

        self.DefRec = RegionReconstruction(args, num_f_prev + 2048)
...
class PointGrid(nn.Module):
    def __init__(self, args, N=16, K=4):
        super(PointGrid, self).__init__()

        # Hyperparameters
        self.N = N  # grid size is N x N x N
        self.K = K  # each cell has K points
        self.NUM_FEATURES = K * 3 + 1

        if N < 16:
            raise ValueError("Require input dimension N>=16.")

        # Encoder
        # Input Dimensions:
        #     B x NUM_FEATURES x N x N x N
        # Encoding Dimensions
        #     B x 512 x N/16 x N/16 x N/16
        self.conv1 = nn.Conv3d(self.NUM_FEATURES, 64, (5, 5, 5), stride=(1, 1, 1), padding=(2, 2, 2))  # N
        self.bn1 = nn.BatchNorm3d(64)
        self.lrelu1 = nn.LeakyReLU(0.1)
        self.conv2 = nn.Conv3d(64, 64, (5, 5, 5), stride=(2, 2, 2), padding=(2, 2, 2))  # N/2
        self.bn2 = nn.BatchNorm3d(64)
        self.lrelu2 = nn.LeakyReLU(0.1)
        self.conv3 = nn.Conv3d(64, 64, (5, 5, 5), stride=(1, 1, 1), padding=(2, 2, 2))
        self.bn3 = nn.BatchNorm3d(64)
        self.lrelu3 = nn.LeakyReLU(0.1)
        self.conv4 = nn.Conv3d(64, 128, (3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1))  # N/4
        self.bn4 = nn.BatchNorm3d(128)
        self.lrelu4 = nn.LeakyReLU(0.1)
        self.conv5 = nn.Conv3d(128, 128, (3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))
        self.bn5 = nn.BatchNorm3d(128)
        self.lrelu5 = nn.LeakyReLU(0.1)
        self.conv6 = nn.Conv3d(128, 256, (3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1))  # N/8
        self.bn6 = nn.BatchNorm3d(256)
        self.lrelu6 = nn.LeakyReLU(0.1)
        self.conv7 = nn.Conv3d(256, 256, (3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))
        self.bn7 = nn.BatchNorm3d(256)
        self.lrelu7 = nn.LeakyReLU(0.1)
        self.conv8 = nn.Conv3d(256, 512, (3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1))  # N/16
        self.bn8 = nn.BatchNorm3d(512)
        self.lrelu8 = nn.LeakyReLU(0.1)
        self.conv9 = nn.Conv3d(512, 512, (1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0))
        self.bn9 = nn.BatchNorm3d(512)
        self.lrelu9 = nn.LeakyReLU(0.1)

        num_f_prev = 64 + 64 + 64 + 128 + 128 + 256 + 256 + 512 + 512
        self.cls_C = class_classifier(args, 512, 10)
        self.domain_C = domain_classifier(args, 512, 2)
        self.posreg_C1 = linear_classifier(512, 4)
        self.posreg_C2 = linear_classifier(512, 4)
        self.defcls_C = ssl_classifier(args, 512, 27)
        self.DecoderFC = DecoderFC(args, 512)
        self.DefRec = RegionReconstruction(args, num_f_prev)

    def forward(self, pointgrid, activate_DefRec=False):
        cls_logits = {}
        # Args:
        #     pointgrid ("BCDHW") : of size B x NUM_FEATURES x N x N x N
        # Returns:
        #     pred_cat ("BC"): of size B x NUM_CATEGORY
        #     pred_seg ("BCDHWK"): of size B x NUM_PART_SEG x N x N x N x (K+1)
        batch_size = pointgrid.size(0)

        # Encoder
        conv1 = self.conv1(pointgrid)
        bn1 = self.bn1(conv1)
        lrelu1 = self.lrelu1(bn1)
        conv2 = self.conv2(lrelu1)
        bn2 = self.bn2(conv2)
        lrelu2 = self.lrelu2(bn2)
        conv3 = self.conv3(lrelu2)
        bn3 = self.bn3(conv3)
        lrelu3 = self.lrelu3(bn3)
        conv4 = self.conv4(lrelu3)
        bn4 = self.bn4(conv4)
        lrelu4 = self.lrelu4(bn4)
        conv5 = self.conv5(lrelu4)
        bn5 = self.bn5(conv5)
        lrelu5 = self.lrelu5(bn5)
        conv6 = self.conv6(lrelu5)
        bn6 = self.bn6(conv6)
        lrelu6 = self.lrelu6(bn6)
        conv7 = self.conv7(lrelu6)
        bn7 = self.bn7(conv7)
        lrelu7 = self.lrelu7(bn7)
        conv8 = self.conv8(lrelu7)
        bn8 = self.bn8(conv8)
        lrelu8 = self.lrelu8(bn8)
        conv9 = self.conv9(lrelu8)
        bn9 = self.bn9(conv9)
        lrelu9 = self.lrelu9(bn9)
        encode = lrelu9.view(batch_size, -1)

        cls_logits["cls"] = self.cls_C(encode)
        cls_logits["domain_cls"] = self.domain_C(encode)
        cls_logits["pos_cls1"] = self.posreg_C1(encode)
        cls_logits["pos_cls2"] = self.posreg_C2(encode)
        cls_logits["def_cls"] = self.defcls_C(encode)
        cls_logits["decoder"] = self.DecoderFC(encode)

        if activate_DefRec:
            DefRec_input = torch.cat((lrelu1, lrelu2, lrelu3, lrelu4, lrelu5, lrelu6, lrelu7, lrelu8, lrelu9), dim=1)
            cls_logits["DefRec"] = self.DefRec(DefRec_input)
